...
public class RenderUtil {
    ...
    public static final ResourceLocation WHITE_OVERLAY = TextureLoader.getLocation("white_overlay");
    ...
    public static void renderTnt(TntFrameData data, PoseStack pMatrixStack, MultiBufferSource pRenderTypeBuffer, int pCombinedLight, boolean pDoFullBright) {
        ...
        if (pDoFullBright) {
            boolean colorful = data.getValue(AdditionType.EMPTY)>0;
            BakedModel overlay = colorful ?
                    Minecraft.getInstance().getModelManager().getModel(Addition.EMPTY.resourceLocation) :
                    Minecraft.getInstance().getModelManager().getModel(WHITE_OVERLAY);
            Minecraft.getInstance().getBlockRenderer().getModelRenderer()
                    .renderModel(pMatrixStack.last(), pRenderTypeBuffer.getBuffer(RenderType.solid()),
                            null, overlay, 1, 1, 1, pCombinedLight, OverlayTexture.NO_OVERLAY,
        ...
        List<BakedQuad> quads = bakedmodel.getQuads
                (state, null, random, new ModelDataMap.Builder()
        ...
        quads.forEach(q -> pRenderTypeBuffer.getBuffer(RenderType.cutout()).putBulkData(pMatrixStack.last(), q, new float[]{1.0F, 1.0F, 1.0F, 1.0F}, 1.0F, 1.0F, 1.0F,
                new int[]{finalPCombinedLight, finalPCombinedLight, finalPCombinedLight, finalPCombinedLight}, OverlayTexture.NO_OVERLAY, true));
    }
